...
namespace WinUtil
{
	namespace Encoding
	{
		static std::string ToBase64(std::vector<unsigned char> const& bytes)
		{
			...
			if (::CryptBinaryToStringA(&bytes[0], static_cast<DWORD>(bytes.size()), CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF, NULL, &requiredChars))
			{
				std::vector<char> buffer(requiredChars, 0);
				if (::CryptBinaryToStringA(&bytes[0], static_cast<DWORD>(bytes.size()), CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF, &buffer[0], &requiredChars))
				{
		...
		static std::vector<unsigned char> FromBase64(std::string const& base64)
		{
			...
			DWORD nBytes = static_cast<DWORD>(data.size());
